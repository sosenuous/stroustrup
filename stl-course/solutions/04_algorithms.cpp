// ============================================================
//  Lesson 4 — <algorithm> : don't reinvent the wheel  ★ SOLUTION ★
// ============================================================
//
...
vector<int> sortedCopy(vector<int> v) {   // note: taken BY VALUE = already a copy
    sort(v.begin(), v.end());             // key idea: sort the local copy ascending
    return v;
}
vector<int> sortedCopyRev(vector<int> v) {   // note: taken BY VALUE = already a copy
    sort(v.begin(), v.end(), greater<int>());  // key idea: greater<> gives descending order
    return v;
}
...
int howMany(const vector<int>& v, int target) {
    // key idea: count_if with a predicate matching target (std::count would also work)
    return count_if(v.begin(), v.end(), [target](int x) {return (x == target);});
}


int howManyIf(const vector<int>& v, bool (*pred)(int)) {
    // key idea: count_if tallies elements satisfying the predicate
...
int largest(const vector<int>& v) {
    // key idea: max_element returns an iterator; dereference it for the value
...
int total(const vector<int>& v) {
    // key idea: accumulate folds the range starting from 0 (adds by default)
...
bool contains(const vector<int>& v, int target) {
    // key idea: find returns end() when absent, so compare against end()
...
bool containsIf(const vector<int>& v, bool (*pred)(int)) {
    // key idea: find_if locates the first match; not end() means one exists
...
pair<int, int> minMax(const vector<int>& v) {
    // key idea: minmax_element yields {min_it, max_it} in one pass; deref both
...
int product(const vector<int>& v) {
    // key idea: fold with multiplication starting from 1 (the identity)
    return accumulate(v.begin(), v.end(), 1, [](int a, int b){ return a * b; });
}
