// ============================================================
//  Lesson 3 — Iterators : how the STL "points" into a container  ★ SOLUTION ★
// ============================================================
//
...
int firstElement(const vector<int>& v) {
    // Dereference the begin() iterator to read the first element.
    return *v.begin();
}
...
int sumWithIterators(const vector<int>& v) {
    int total = 0;
    // Walk begin()..end(), adding each dereferenced element to the total.
    for (auto it = v.begin(); it != v.end(); ++it) {
        total += *it;
    }
    return total;
...
int sumWithFold(const vector<int>& v) {
    // accumulate folds the range with a binary lambda, starting from 0.
...
int maxWithFold(const vector<int>& v) {
    // Fold seeded with the first element, keeping the larger of acc and x each step.
    return accumulate(v.begin(), v.end(), *v.begin(),
...
vector<int> reversedCopy(const vector<int>& v) {
    // Range-construct from the reverse-iterator pair to copy back-to-front.
...
int indexOfMax(const vector<int>& v) {
    // max_element yields an iterator to the first max; distance turns it into an index.
